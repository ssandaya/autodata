...
class TestDownload < Test::Unit::TestCase
  ...
  def test_download
    ...
    sleep 30
    browser.select_list(:class => "gwt-ListBox").select "Thumbdrive"
    sleep 30
    browser.button(:text => "Download").wait_until_present(timeout: 120)
    sleep 10
    ...
    sleep 300
    browser.div(:text => "Download Complete").wait_until_present(timeout: 900)
    sleep 30
