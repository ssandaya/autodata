...
class Test::Unit::TestCase
  def assert_equal(expected, got, msg, browser)
    begin
      ...
      filename = DateTime.now.strftime("450Plus-Activate" +"-10.20.95.65-" + "%Y%m%d-%H%M%S")
...
class TestActivate < Test::Unit::TestCase
  ...
  def test_activate
    ...
    assert_equal(txt_current_version,"7.HB 331.28","Oooops! Current version incorrect", browser)
    ...
    assert_equal(txt_available_version,"7.HB 331.29","Oooops! Available version incorrect", browser)
    puts "Available Version: " + txt_available_version
    sleep 10
    
    assert_equal(browser.button(:text, "Activate").present?, true, "Ooooops! Activate button not found", browser)
    sleep 10
