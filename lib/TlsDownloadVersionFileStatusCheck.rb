...
class TlsDownloadVersionFileStatusCheck_4B < Test::Unit::TestCase
  ...
  def test_tls_download_version_file_status_check
    puts "Tls Version Download Status Check"
    puts "Today " + DateTime.now.strftime("%d/%m/%Y %H:%M:%S:%s")
    ...
    puts SERVICES['tls_version_to']
    assert_equal_browser(SERVICES['tls_version_to'], txt_available_version,"Oooops2! Incorrect upgrade version", browser)
    puts "Available Version: " + txt_available_version
    sleep 10
    
    assert_equal_browser(browser.button(:text, "Activate").present?, true, "Ooooops3! Incorrect button action.", browser)
    sleep 10
