...
class Test::Unit::TestCase
  filepath = File.dirname(__FILE__) + "/config/tlscheckbuilddate_2C.yml"
...
class TlsDownloadVersionFileStatusCheck_4B < Test::Unit::TestCase
  ...
  def test_tls_download_version_file_status_check_4b
    ...
    browser.goto SERVICES['url_base']
    sleep 15
    browser.div(:text, "System Status").wait_until(&:present?).click
    sleep 10
    browser.div(:text, "About").wait_until(&:present?).click
    sleep 10
  
    puts "Expected Build Date: " + SERVICES['build_date']
    build_txt = browser.span(:id, "gwt-debug-softwareBuildDate").text
    
    puts "Build Date         : " + build_txt
    assert_equal_browser(SERVICES['build_date'], build_txt, "Oooops! Incorrect current version", browser)	
