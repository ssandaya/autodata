...
class Test::Unit::TestCase
  filepath = File.dirname(__FILE__) + "/config/tls_default107.yml"
  ...
  def assert_equal_browser(expected, got, msg, browser)
    begin
      ...
      filename = DateTime.now.strftime("TLS-FetchCreateSD-#{SERVICES['ip_address']}-%Y%m%d-%H%M%S")
...
class DatabaseInitTest < Test::Unit::TestCase
  ...
  def test_database_initialization
    ...
    system("C:/cygwin64/bin/ssh root@#{SERVICES['ip_address']} '[ -d /tmp/stage ] && (cd /tmp/stage && ./create.sh --ip 10.20.95.107 --gw 10.20.95.1 --dns 10.20.77.54 --hostname satls450p2 ; sleep 10)'")

    sleep 10
    system ("C:/cygwin64/bin/ssh root@#{SERVICES['ip_address']} '[ -d /tmp/stage ] && (cd /tmp/stage && /etc/init.d/applications start ; sleep 10)'")
